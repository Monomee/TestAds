...
public class GoogleAdsManager : MonoBehaviour
{
    ...
    {
        // Initialize the Google Mobile Ads SDK.
        // XÓA DÒNG NÀY: MobileAds.RaiseAdEventsOnUnityMainThread = true;

        MobileAds.Initialize(initStatus =>
        {
            // Chạy các logic load quảng cáo bên trong này
            // Lưu ý: Các callback của Google Ads hiện nay đa số đã tự chạy trên Main Thread
            LoadBannerAd();
            LoadInterstitialAd();

            // Đăng ký sự kiện
            interRewardEvent -= GiveInterReward; // Tránh đăng ký trùng lặp
            interRewardEvent += GiveInterReward;
